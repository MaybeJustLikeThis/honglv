...
import {
  ...
  postLogin,
  postV,
} from "@/utils/api/adminApi/Enroll.js";
...
function login() {
  postLogin({
    ...
    localStorage.setItem("Honglv_token", res.data.token);
    console.log(res, "登录信息1");
    router.push({ path: "/honglv" });
  });
}
      ...
      <div class="content">
        ...
        <div class="confirm-protocol">
          <div class="login-next">
            <input type="radio" class="confirm-protocoled" />下次自动登录
          </div>
          <div class="forget-password">忘记密码</div>
          <div class="login-change" @click="router.push({ path: '/enroll' })">
            注册
          </div>
        </div>
        <div class="other">
